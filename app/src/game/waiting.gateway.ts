...
export class GameWaitingGateway {
    constructor(private auth: AuthSocketService, private sockets: SocketsService, private game: GameService, private matches: MatchesService) {
    }

    // Tell the server the user is ready for the game to start.
    @SubscribeMessage("game.waiting.ready")
    async iamready(@ConnectedSocket() socket: WebSocket, @MessageBody() body: GameWaitingDto) {
        ...
        let match = body.id === 0 ? undefined : await this.matches.get({ where: { id: body.id } })
        ...
        if (sockets.length === 0) {
            ...
            // register the socket.
            if (body.id === 0 || (!match && match.state === MatchState.WAITING)) {
        ...
        for (const s of [waiting, socket]) {
            s.send(JSON.stringify({ event: "game.waiting.success", data: { id: match.id } }))
        }

        const [playerOne, playerTwo] = this.auth.socketUserId(waiting) === match.playerOneId
            ? [waiting, socket]
            : [socket, waiting]
    ...
    iamunready(@ConnectedSocket() socket: WebSocket, @MessageBody() body: GameWaitingDto) {
