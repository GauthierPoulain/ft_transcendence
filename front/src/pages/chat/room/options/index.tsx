    ...
    return (
        <div className="d-flex align-items-stretch gap-x-2">
            {channel.type !== "direct" && (
                <>
                    <LeaveChannelButton />
                    {channel.type !== "public" && member.role === "owner" && (
                        <MakePublicButton channelId={channel.id} />
                    )}
                    {channel.type !== "private" && member.role === "owner" && (
                        <MakePrivateButton channelId={channel.id} />
                    )}

                    {member.role === "owner" && (
                        <MakeProtectedButton channelId={channel.id} />
                    )}
                </>
            )}
        </div>
